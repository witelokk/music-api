...
func TestPlaylistsService_GetPlaylistSongs_NotFound(t *testing.T) {
	repo := &fakePlaylistsRepo{
		playlistErr: ErrPlaylistNotFound,
	}
	svc := NewPlaylistsService(repo)

	songs, err := svc.GetPlaylistSongs(context.Background(), "user-id", "playlist-id")
	if !errors.Is(err, ErrPlaylistNotFound) {
		t.Fatalf("expected ErrPlaylistNotFound, got %v", err)
	}
	if songs != nil {
		t.Fatalf("expected nil songs, got %+v", songs)
	}
}

func TestPlaylistsService_GetPlaylistSongs_EmptyPlaylist(t *testing.T) {
	repo := &fakePlaylistsRepo{
		playlist: &Playlist{
			ID:     "playlist-id",
			UserID: "user-id",
	...
	if len(songs) != 0 {
		t.Fatalf("expected empty songs list, got %+v", songs)
	}
}

func TestPlaylistsService_AddSongToPlaylist_NotFound(t *testing.T) {
	repo := &fakePlaylistsRepo{
		addErr: ErrPlaylistNotFound,
	}
	svc := NewPlaylistsService(repo)
...
func TestPlaylistsService_AddSongToPlaylist_SongNotFound(t *testing.T) {
	repo := &fakePlaylistsRepo{
